...
export class QuantizerWsmeans {
	...
	static quantize(
		...
		let pointCount = 0;
		for (let i = 0; i < inputPixels.size(); i++) {
		...
		for (let i = 0; i < startingClusters.size(); i++) {
			clusters.push(pointProvider.fromInt(startingClusters[i]));
		}
		const additionalClustersNeeded = clusterCount - clusters.size();
		if (startingClusters.size() === 0 && additionalClustersNeeded > 0) {
		...
		for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
			...
			const componentASums = new Array<number>(clusterCount, 0);
			const componentBSums = new Array<number>(clusterCount, 0);
			const componentCSums = new Array<number>(clusterCount, 0);
