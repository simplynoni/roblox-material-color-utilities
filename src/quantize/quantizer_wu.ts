...
export class QuantizerWu {
	...
	private constructHistogram(pixels: number[]) {
		this.weights = new Array<number>(TOTAL_SIZE, 0);
		this.momentsR = new Array<number>(TOTAL_SIZE, 0);
		this.momentsG = new Array<number>(TOTAL_SIZE, 0);
		this.momentsB = new Array<number>(TOTAL_SIZE, 0);
		this.moments = new Array<number>(TOTAL_SIZE, 0);
	...
	private createBoxes(maxColors: number): CreateBoxesResult {
		...
		for (let i = 1; i < maxColors; i++) {
			if (this.cut(this.cubes[_next], this.cubes[i])) {
				volumeVariance[_next] = this.cubes[_next].vol > 1 ? this.variance(this.cubes[_next]) : 0.0;
				volumeVariance[i] = this.cubes[i].vol > 1 ? this.variance(this.cubes[i]) : 0.0;
			} else {
				volumeVariance[_next] = 0.0;
				i--;
			}
