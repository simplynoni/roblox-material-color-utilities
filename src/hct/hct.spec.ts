...
describe('CAM to ARGB to CAM', () => {
	it('red', () => {
		...
		expect(argb).toBe(RED);
	});

	it('green', () => {
		const cam = Cam16.fromInt(GREEN);
		const argb = cam.toInt();
		expect(argb).toBe(GREEN);
	});

	it('blue', () => {
		const cam = Cam16.fromInt(BLUE);
		const argb = cam.toInt();
		expect(argb).toBe(BLUE);
	});
});
...
describe('ARGB to HCT', () => {
	it('green', () => {
		...
		expect(hct.getHue()).toBeCloseTo(142.139, 2);
		expect(hct.getChroma()).toBeCloseTo(108.41, 2);
		expect(hct.getTone()).toBeCloseTo(87.737, 2);
	});

	it('blue', () => {
		const hct = Hct.fromInt(BLUE);
		expect(hct.getHue()).toBeCloseTo(282.788, 2);
		expect(hct.getChroma()).toBeCloseTo(87.23, 2);
		expect(hct.getTone()).toBeCloseTo(32.302, 2);
	});

	it('blue tone 90', () => {
		const hct = Hct.from(282.788, 87.23, 90.0);
		expect(hct.getHue()).toBeCloseTo(282.239, 2);
		expect(hct.getChroma()).toBeCloseTo(19.144, 2);
		expect(hct.getTone()).toBeCloseTo(90.035, 2);
	});
});
		...
		for (let hue = 15; hue < 360; hue += 30) {
			for (let chroma = 0; chroma <= 100; chroma += 10) {
				for (let tone = 20; tone <= 80; tone += 10) {
					...
					expect(math.abs(hctColor.getTone() - tone)).toBeLessThanOrEqual(0.5);
				}
			}
		}
	});
