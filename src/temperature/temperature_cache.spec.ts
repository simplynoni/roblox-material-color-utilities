...
describe('TemperatureCache', () => {
	...
	it('relative temperature', () => {
		const blueTemp = new TemperatureCache(Hct.fromInt(0xff0000ff)).getInputRelativeTemperature();
		expect(blueTemp).toBeCloseTo(0.0, 3);

		const redTemp = new TemperatureCache(Hct.fromInt(0xffff0000)).getInputRelativeTemperature();
		expect(redTemp).toBeCloseTo(1.0, 3);

		const greenTemp = new TemperatureCache(Hct.fromInt(0xff00ff00)).getInputRelativeTemperature();
		expect(greenTemp).toBeCloseTo(0.467, 3);

		const whiteTemp = new TemperatureCache(Hct.fromInt(0xffffffff)).getInputRelativeTemperature();
		expect(whiteTemp).toBeCloseTo(0.5, 3);

		const blackTemp = new TemperatureCache(Hct.fromInt(0xff000000)).getInputRelativeTemperature();
		expect(blackTemp).toBeCloseTo(0.5, 3);
	});

	it('complement', () => {
		const blueComplement = new TemperatureCache(Hct.fromInt(0xff0000ff)).getComplement().toInt();
		expect(blueComplement).toBe(0xff9d0002);

		const redComplement = new TemperatureCache(Hct.fromInt(0xffff0000)).getComplement().toInt();
		expect(redComplement).toBe(0xff007bfc);

		const greenComplement = new TemperatureCache(Hct.fromInt(0xff00ff00)).getComplement().toInt();
		expect(greenComplement).toBe(0xffffd2c9);

		const whiteComplement = new TemperatureCache(Hct.fromInt(0xffffffff)).getComplement().toInt();
		expect(whiteComplement).toBe(0xffffffff);

		const blackComplement = new TemperatureCache(Hct.fromInt(0xff000000)).getComplement().toInt();
		expect(blackComplement).toBe(0xff000000);
	});

	it('analogous', () => {
		const blueAnalogous = new TemperatureCache(Hct.fromInt(0xff0000ff)).analogous().map((e) => e.toInt());
		...
		const redAnalogous = new TemperatureCache(Hct.fromInt(0xffff0000)).analogous().map((e) => e.toInt());
		...
		const greenAnalogous = new TemperatureCache(Hct.fromInt(0xff00ff00)).analogous().map((e) => e.toInt());
		...
		const blackAnalogous = new TemperatureCache(Hct.fromInt(0xff000000)).analogous().map((e) => e.toInt());
		...
		const whiteAnalogous = new TemperatureCache(Hct.fromInt(0xffffffff)).analogous().map((e) => e.toInt());
