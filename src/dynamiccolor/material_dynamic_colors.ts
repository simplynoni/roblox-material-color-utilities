...
function findDesiredChromaByTone(hue: number, chroma: number, tone: number, byDecreasingTone: boolean): number {
	...
	if (closestToChroma.getChroma() < chroma) {
		let chromaPeak = closestToChroma.getChroma();
		while (closestToChroma.getChroma() < chroma) {
			...
			const potentialDelta = math.abs(potentialSolution.getChroma() - chroma);
			const currentDelta = math.abs(closestToChroma.getChroma() - chroma);
...
export class MaterialDynamicColors {
	...
	static primary = DynamicColor.fromPalette({
		...
		toneDeltaPair: (s) =>
			new ToneDeltaPair(MaterialDynamicColors.primaryContainer, MaterialDynamicColors.primary, 10, 'nearer', false),
	});
	...
	static secondary = DynamicColor.fromPalette({
		...
		contrastCurve: new ContrastCurve(3, 4.5, 7, 7),
		toneDeltaPair: (s) =>
			new ToneDeltaPair(MaterialDynamicColors.secondaryContainer, MaterialDynamicColors.secondary, 10, 'nearer', false),
	});
	...
	static tertiary = DynamicColor.fromPalette({
		...
		contrastCurve: new ContrastCurve(3, 4.5, 7, 7),
		toneDeltaPair: (s) =>
			new ToneDeltaPair(MaterialDynamicColors.tertiaryContainer, MaterialDynamicColors.tertiary, 10, 'nearer', false),
	});
	...
	static tertiaryContainer = DynamicColor.fromPalette({
		...
		tone: (s) => {
			...
			const proposedHct = s.tertiaryPalette.getHct(s.sourceColorHct.getTone());
	...
	static error = DynamicColor.fromPalette({
		...
		contrastCurve: new ContrastCurve(3, 4.5, 7, 7),
		toneDeltaPair: (s) =>
			new ToneDeltaPair(MaterialDynamicColors.errorContainer, MaterialDynamicColors.error, 10, 'nearer', false),
	});
	...
	static primaryFixed = DynamicColor.fromPalette({
		...
		contrastCurve: new ContrastCurve(1, 1, 3, 4.5),
		toneDeltaPair: (s) =>
			new ToneDeltaPair(MaterialDynamicColors.primaryFixed, MaterialDynamicColors.primaryFixedDim, 10, 'lighter', true),
	});
