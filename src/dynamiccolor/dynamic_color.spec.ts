...
import type { DynamicScheme } from '../scheme/dynamic_scheme';
...
import { toFixed } from '../utils/math_utils';
import { ContrastCurve } from './contrast_curve';
import { MaterialDynamicColors } from './material_dynamic_colors';

const seedColors = [Hct.fromInt(0xffff0000), Hct.fromInt(0xffffff00), Hct.fromInt(0xff00ff00), Hct.fromInt(0xff0000ff)];
...
for (const color of seedColors) {
	for (const contrastLevel of [-1.0, -0.75, -0.5, -0.25, 0.0, 0.25, 0.5, 0.75, 1.0]) {
	...
	it('generates colors respecting contrast', () => {
		for (const scheme of schemes) {
			for (const pair of textSurfacePairs) {
				...
				const foregroundTone = colorByName.get(fgName)!.getHct(scheme).getTone();
				// biome-ignore lint/style/noNonNullAssertion: <explanation>
				const backgroundTone = colorByName.get(bgName)!.getHct(scheme).getTone();
				...
				if (contrast < minimumRequirement) {
					error(`${fgName} on ${bgName} is ${contrast}, needed ${minimumRequirement}`);
				}
			}
		}
	});
    ...
    */
		...
		const constraints = [
			...
			// Delta constraints.
			{
				...
				respectively: true,
				fore: ['primary', 'secondary', 'tertiary', 'error'],
				back: ['primary_container', 'secondary_container', 'tertiary_container', 'error_container'],
				polarity: 'farther',
			},
			{
				kind: 'Delta',
		...
		for (const scheme of schemes) {
			...
			const resolvedColors = new Map(colors.map((color) => [color.name, color.getArgb(scheme)]));

			for (const cstr of constraints) {
				if (cstr.kind === 'Contrast') {
					const contrastTolerance = 0.05;

					const minRequirement = getMinRequirement(cstr.values!, scheme.contrastLevel);
					const respectively = cstr.respectively ?? false;
					const _pairs = getPairs(respectively, cstr.fore!, cstr.back!);
					// Check each pair
					for (const pair of _pairs) {
						...
						if (contrast < minRequirement - contrastTolerance && minRequirement <= 4.5) {
							// Real fail.
							error(
								`Contrast ${fore} ${toFixed(ftone, prec)} ${back} ${toFixed(btone, prec)} ${toFixed(
									contrast,
									prec,
								)} ${minRequirement} `,
							);
						...
						if (failing && minRequirement > 4.5) {
							error(
								`Contrast(stretch-goal) ${fore} ${toFixed(ftone, prec)} ${back} ${toFixed(btone, prec)} ${toFixed(
									contrast,
									prec,
								)} ${minRequirement} `,
							);
					...
					expect(
						polarity === 'nearer' || polarity === 'farther' || polarity === 'lighter' || polarity === 'darker',
					).toBe(true);
					for (const pair of _pairs) {
						const [fore, back] = pair;
						const ftone = colorUtils.lstarFromArgb(resolvedColors.get(fore)!);
						...
						if (observedDelta < cstr.delta! - 0.5 /* lenient */) {
							// Failing
							error(
								`Delta ${fore} ${toFixed(ftone, prec)} ${back} ${toFixed(btone, prec)} ${toFixed(
									observedDelta,
									prec,
								)} ${cstr.delta}`,
							);
					...
					for (const bg of cstr.objects!) {
						...
						if (bgtone >= 50.5 && bgtone < 59.5 /* lenient */) {
							// Failing
							error(`Background ${bg} ${toFixed(bgtone, prec)}`);
						}
					}
				} else {
					error(`Bad constraint kind = ${cstr.kind}`);
				}
			}
		}
	});
