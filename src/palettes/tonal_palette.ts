...
export class TonalPalette {
	...
	private static createKeyColor(hue: number, chroma: number): Hct {
		...
		let smallestDelta = math.abs(smallestDeltaHct.getChroma() - chroma);
		...
		for (let delta = 1.0; delta < 50.0; delta += 1.0) {
			...
			const hctAddDelta = math.abs(hctAdd.getChroma() - chroma);
			...
			const hctSubtractDelta = math.abs(hctSubtract.getChroma() - chroma);
