...
export class Score {
	...
	static score(colorsToPopulation: Map<number, number>, options?: ScoreOptions): number[] {
		...
		// total count.
		const colorsHct: Hct[] = [];
		const huePopulation = new Array<number>(360, 0);
		let populationSum = 0;
		for (const [argb, population] of pairs(colorsToPopulation)) {
			const hct = Hct.fromInt(argb);
			colorsHct.push(hct);
			const hue = math.floor(hct.getHue());
			huePopulation[hue] += population;
			populationSum += population;
		}

		// Hues with more usage in neighboring 30 degree slice get a larger number.
		const hueExcitedProportions = new Array<number>(360, 0.0);
		for (let hue = 0; hue < 360; hue++) {
			const proportion = huePopulation[hue] / populationSum;
			for (let i = hue - 14; i < hue + 16; i++) {
				const neighborHue = mathUtils.sanitizeDegreesInt(i);
		...
		for (const hct of colorsHct) {
			const hue = mathUtils.sanitizeDegreesInt(math.round(hct.getHue()));
			const proportion = hueExcitedProportions[hue];
			if (filter && (hct.getChroma() < Score.CUTOFF_CHROMA || proportion <= Score.CUTOFF_EXCITED_PROPORTION)) {
				continue;
			}

			const proportionScore = proportion * 100.0 * Score.WEIGHT_PROPORTION;
			const chromaWeight =
				hct.getChroma() < Score.TARGET_CHROMA ? Score.WEIGHT_CHROMA_BELOW : Score.WEIGHT_CHROMA_ABOVE;
			const chromaScore = (hct.getChroma() - Score.TARGET_CHROMA) * chromaWeight;
		...
		for (let differenceDegrees = 90; differenceDegrees >= 15; differenceDegrees--) {
			...
			for (const { hct } of scoredHct) {
				const duplicateHue = chosenColors.find((chosenHct) => {
					return mathUtils.differenceDegrees(hct.getHue(), chosenHct.getHue()) < differenceDegrees;
				});
				if (!duplicateHue) {
